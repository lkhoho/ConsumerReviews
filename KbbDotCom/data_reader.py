...
def readReviewData(filename: str) -> tuple:
    ...
    scores = df[config.kbbDotComLabelConfig['scoreName']]
    reviews = [x.strip().replace('\n', ' ') for x in df['content'].tolist()]
    posneg = [1 if score >= config.kbbDotComLabelConfig['threshold'] else 0 for score in scores]
