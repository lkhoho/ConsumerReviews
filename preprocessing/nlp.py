...
def compute_sentiment_scores_all_words(cleaned_texts: list, posneg: list, word2score: dict, stopwords: set,
                                       label_name='posneg', save_raw=True, raw_file_path=None,
                                       compute_mul=True, mul_file_path=None, compute_score=True, score_file_path=None):
    """
    Compute sentiment scores for all words.
    """

    log = get_logger('compute_sentiment_scores_all_words')
    ...
    posneg = pd.Series(posneg)
    df[label_name] = posneg
    if save_raw:
        filename = raw_file_path + '_raw_tfidf.csv'
        df.to_csv(filename, index=False)
        log.info('TFIDF of shape {} is saved in {}.'.format(df.shape, filename))
        ...
        for feature in df:
            if feature in word2score:
                ...
                if score == word2score[feature][0]:
                    ...
                    df[feature] *= 0.0
        filename = mul_file_path + '_raw_mul.csv'
        df.to_csv(filename, index=False)
        log.info('TFIDF of shape {} is saved in {}.'.format(df.shape, filename))
    ...
    if compute_score:
        ...
        df[label_name] = posneg
        filename = score_file_path + '_raw_sc.csv'
        df.to_csv(filename, index=False)
        log.info('TFIDF of shape {} is saved in {}.'.format(df.shape, filename))


def compute_sentiment_scores_nouns(cleaned_texts: list, posneg: list, word2score: dict, stopwords: set,
                                   label_name='posneg', save_noun_scores=False, noun_score_file_path=None,
                                   save_raw=True, raw_file_path=None, compute_mul=True, mul_file_path=None,
                                   compute_score=True, score_file_path=None):
    """
    Compute sentiment scores for nouns only.
    """

    log = get_logger('compute_sentiment_scores_nouns')

    # 1) get scores of nouns
    noun_scores = _get_noun_scores(cleaned_texts, word2score)
    if save_noun_scores:
        filename = noun_score_file_path + '_sentiment_scores_nouns.json'
        with open(filename, 'w') as fp:
            simplejson.dump(noun_scores, fp)
        assert len(noun_scores) == len(cleaned_texts)
        log.info('Sentiment scores of nouns are saved in {}.'.format(filename))
    ...
    df[label_name] = posneg
    if save_raw:
        filename = raw_file_path + '_nouns_tfidf.csv'
        df.to_csv(filename, index=False)
        log.info('TFIDF of shape {} is saved in {}.'.format(df.shape, filename))
    ...
    if compute_mul:
        ...
        for i in range(df.shape[0]):
            for noun, score in noun_scores[i].items():
                df.loc[i, noun] *= score
        filename = mul_file_path + '_nouns_mul.csv'
        df.to_csv(filename, index=False)
        log.info('TFIDF of shape {} is saved in {}.'.format(df.shape, filename))
    ...
    if compute_score:
        ...
        df = pd.DataFrame(pure_scores)
        df[label_name] = posneg
        filename = score_file_path + '_nouns_sc.csv'
        df.to_csv(filename, index=False)
        log.info('TFIDF of shape {} is saved in {}.'.format(df.shape, filename))


def compute_sentiment_scores_transform_objective_words(cleaned_texts: list, lemmatized_texts: list, posneg: list,
                                                       word2score: dict, stopwords: set, label_name='posneg', sep='###',
                                                       save_raw=True, raw_file_path=None,
                                                       save_mul=True, mul_file_path=None,
                                                       compute_score=True, score_file_path=None):
    """
    Compute sentiment scores based on transformed objective words.
    """

    log = get_logger('compute_sentiment_scores_transform_objective_words')
    ...
    df[label_name] = posneg
    if save_raw:
        filename = raw_file_path + '_all_tfidf.csv'
        df.to_csv(filename, index=False)
    ...
    if save_mul:
        filename = mul_file_path + '_all_mul.csv'
        df.to_csv(filename, index=False)
        log.info('TFIDF of shape {} is saved in {}.'.format(df.shape, filename))
    ...
    if compute_score:
        ...
        df = pd.DataFrame(pure_scores)
        df[label_name] = posneg
        filename = score_file_path + '_all_sc.csv'
        df.to_csv(filename, index=False)
        log.info('TFIDF of shape {} is saved in {}.'.format(df.shape, filename))
