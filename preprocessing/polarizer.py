...
class Polarizer(object):
    ...
    def _aspect_review_dict(self, corpus: ReviewSents, aspect: str):
        '''
        ...
        for s, w in zip(sent_idx, word_pos_idx):
            review = corpus.sentences[s].review_id
            ...
            if s == prev_sent:
                ...
                rating = corpus.sentences[s].review_rating
                ...
                if review_dict[review]['first_aspect_idx'] is None:
    ...
    def _polarity_class(self, aspect, review):
        ...
        # review_txt = unicodedata.normalize('NFKD', review_txt).encode('ascii', 'ignore')
    ...
    def _top_aspects(self):
        ...
        top_asps = sorted(zip(asps, aspects_rev_f), key=lambda x: x[1], reverse=True)
        top_asps = [[asp[0] for asp in top_asps], [asp[1] for asp in top_asps]]

        self.top_asps = top_asps

    def polarize_aspects(self, corpus: ReviewSents, aspect_list: List[str] = None):
        '''
        ...
        for category, label, total_lines in categories:
            ...
            for txt, asp_idx, _, _, _ in dic[category]:
                ...
                while len(frag) > max_txt_len:
                    ...
                    lst = map(len, chars)
                    stacked = np.hstack([0, np.cumsum(lst)])
                    lst = np.arange(stacked.shape[0]) + stacked
